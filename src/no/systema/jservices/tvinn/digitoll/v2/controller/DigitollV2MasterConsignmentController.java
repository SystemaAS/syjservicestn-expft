...
import no.systema.jservices.tvinn.expressfortolling2.dto.MasterConsignments;
...
public class DigitollV2MasterConsignmentController {
					...
					for (SadmomfDto dto: list) {
						...
						if(StringUtils.isEmpty(dto.getEmmid()) ) {
							...
							if(isApiAir) {
								...
								json = apiServicesRail.postMasterConsignmentDigitollV2(mc, tollTokenMap);
							}else {
							...
							if(StringUtils.isEmpty(obj.getRequestId())){
								...
								//GET MRN right here...
								String mrn = poolExecutorControllerService.getMrnPOSTDigitollV2FromApi(dtoResponse, dtoResponse.getRequestId(), dto.getEmuuid_own(), tollTokenMap, isApiAir, isApiRail, 
					...
					for (SadmomfDto dto: list) {
						...
						if(StringUtils.isNotEmpty(dto.getEmmid()) ) {
							...
							if(isApiAir) {
								...
								json = apiServicesRail.putMasterConsignmentDigitollV2(mc, mrn, tollTokenMap);
							}else {
							...
							if(StringUtils.isEmpty(obj.getRequestId())){
								...
								if(StringUtils.isNotEmpty(requestId) && StringUtils.isNotEmpty(mrn)) {
									...
									this.checkLrnValidationStatusMasterConsignmentDigitollV2FromApi(dtoResponse, requestId, tollTokenMap, isApiAir, isApiRail);
									if(StringUtils.isNotEmpty(dtoResponse.getErrMsg())){
					...
					for (SadmomfDto dto: list) {
						...
						if(StringUtils.isNotEmpty(dto.getEmmid() )) {
							...
							if(isApiAir) {
								...
								json = apiServicesRail.deleteMasterConsignmentDigitollV2(mc, mrn);
							}else {
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/digitollv2/getDocsRecMasterConsignment_withDescendants.do", method={RequestMethod.GET, RequestMethod.POST}) 
	@ResponseBody
	public GenericWithDescendantsDtoContainer getDocsReceivedMasterConsignmentWithDescendantsDigitollV2(HttpServletRequest request , @RequestParam(value = "user", required = true) String user, 
																				@RequestParam(value = "mrn", required = true) String mrn ) throws Exception {
		...
		try {
			if(checkUser(user)) {
				logger.warn("user OK:" + user);
				//API - PROD
				String json = apiServices.getDocsReceivedMasterConsignmentWithDescendantsDigitollV2(mrn);
				logger.warn("JSON = " + json);
				if(StringUtils.isNotEmpty(json)) {
					
					MasterConsignments dto = new ObjectMapper().readValue(json, MasterConsignments.class);
					if(dto!=null) {
						logger.info(dto.toString());
	...
	@RequestMapping(value="/digitollv2/getDocsRecMasterConsignmentRail.do", method={RequestMethod.GET, RequestMethod.POST}) 
	@ResponseBody
	public GenericDtoResponseLight getDocsReceivedMasterConsignmentRailDigitollV2(HttpServletRequest request , @RequestParam(value = "user", required = true) String user, 
																				@RequestParam(value = "mrn", required = true) String mrn ) throws Exception {
		...
		try {
			if(checkUser(user)) {
				...
				String json = apiServicesRail.getDocsReceivedMasterConsignmentDigitollV2(mrn);
				logger.warn("JSON = " + json);
				if(StringUtils.isNotEmpty(json)) {
					ApiMrnStatusRecordDto[] obj = new ObjectMapper().readValue(json, ApiMrnStatusRecordDto[].class);
					if(obj!=null) {
	...
	@RequestMapping(value="/digitollv2/getDocsRecMasterConsignmentRail_withDescendants.do", method={RequestMethod.GET, RequestMethod.POST}) 
	@ResponseBody
	public GenericWithDescendantsDtoContainer getDocsReceivedMasterConsignmentRailWithDescendantsDigitollV2(HttpServletRequest request , @RequestParam(value = "user", required = true) String user, 
																@RequestParam(value = "mrn", required = true) String mrn ) throws Exception {
		...
		try {
			if(checkUser(user)) {
				logger.warn("user OK:" + user);
				//API - PROD
				String json = apiServicesRail.getDocsReceivedMasterConsignmentWithDescendantsDigitollV2(mrn);
				logger.warn("JSON = " + json);
				if(StringUtils.isNotEmpty(json)) {
	...
	@RequestMapping(value="/digitollv2/getDocsRecMasterConsignmentAir.do", method={RequestMethod.GET, RequestMethod.POST}) 
	@ResponseBody
	public GenericDtoResponseLight getDocsReceivedMasterConsignmentAirDigitollV2(HttpServletRequest request , @RequestParam(value = "user", required = true) String user, 
																				@RequestParam(value = "mrn", required = true) String mrn ) throws Exception {
		...
		try {
			if(checkUser(user)) {
				...
				String json = apiServicesAir.getDocsReceivedMasterConsignmentDigitollV2(mrn);
				logger.warn("JSON = " + json);
				if(StringUtils.isNotEmpty(json)) {
					ApiMrnStatusRecordDto[] obj = new ObjectMapper().readValue(json, ApiMrnStatusRecordDto[].class);
					if(obj!=null) {
	...
	@RequestMapping(value="/digitollv2/getDocsRecMasterConsignmentAir_withDescendants.do", method={RequestMethod.GET, RequestMethod.POST}) 
	@ResponseBody
	public GenericWithDescendantsDtoContainer getDocsReceivedMasterConsignmentAirWithDescendantsDigitollV2(HttpServletRequest request , @RequestParam(value = "user", required = true) String user, 
																@RequestParam(value = "mrn", required = true) String mrn ) throws Exception {
		...
		try {
			if(checkUser(user)) {
				logger.warn("user OK:" + user);
				//API - PROD
				String json = apiServicesAir.getDocsReceivedMasterConsignmentWithDescendantsDigitollV2(mrn);
				logger.warn("JSON = " + json);
				if(StringUtils.isNotEmpty(json)) {
	...
	private String getMrnMasterConsignmentDigitollV2FromApi(GenericDtoResponse dtoResponse, String lrn, String apiType) {
		...
		try{
			...
			if(StringUtils.isNotEmpty(apiType) && apiType.equalsIgnoreCase("air")) {
				...
				json = apiServicesRail.getValidationStatusMasterConsignmentDigitollV2(lrn);
			}else {
	 ...
	 * @return
	 */
	private String checkLrnValidationStatusMasterConsignmentDigitollV2FromApi(GenericDtoResponse dtoResponse, String lrn, Map tollTokenMap, boolean isApiAir, boolean isApiRail) {
